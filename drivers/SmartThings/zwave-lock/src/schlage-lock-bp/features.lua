...
function M.configuration_report(_, device, cmd)
...
function M.refresh_settings(_, device)
...
function M.setting_command(_, device, cmd)
  local spec = M.command_specs[cmd.capability] and M.command_specs[cmd.capability][cmd.command]
...
function M.activity_from_notification(_, device, cmd, user_lookup)
  ...
  local detail = activity_map[cmd.args.event]
  if not detail then return end
  if cmd.args.event == access.KEYPAD_LOCK_OPERATION or cmd.args.event == access.KEYPAD_UNLOCK_OPERATION then
